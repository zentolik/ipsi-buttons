    ...
    if (HOST === 'otrs.euroweb.net' && URL[1] && !document.body.classList.contains('LoginScreen')) { // Code nur laufen lassen; wenn NICHT im LogIn-Screen, es die OTRS-Seite ist und die URL die MailData beinhaltet.
        ...
        (function createMailSettingsPopup() {
            ...
            if (department_array[0].includes('webdepartment berlin')) {
                ...
                domainSelect.name = 'domain_select'
                domainSelect.innerHTML = selectElement;
                ...
                domainInput.id = 'wishdomain';
                domainInput.name = 'wishdomain';
                domainInput.oninput = () => {
                ...
                qualitySelect.name = 'imgquality_select'
                qualitySelect.innerHTML = selectElement;
                ...
                qrSelect.name = 'qrcode_select'
                qrSelect.innerHTML = selectElement;
                qrSelect.value = mailSettings.qr_code;
                qrSelect.onchange = () => {
                    mailSettings.qr_code = qrSelect.value;
                };
                qrLabel.appendChild(qrSelect);
                form.appendChild(qrLabel);
            }
                    ...
                    setTimeout(() => {
                        if (department_array[0].includes('webdepartment berlin')) {
                            ...
                            (function cleanUpBody() {
                                ...
                                // QR-Code (ja/nein)
                                if (mailSettings.qr_code === 'no') {
                                    html = html.replace(/<p>Gerne erstellen wir für Sie noch den gewünschten QR-Code. Bitte teilen Sie uns mit, was der QR-Code beinhalten soll und in welchem Format Sie diesen benötigen.<\/p>/g, '');
                                }
