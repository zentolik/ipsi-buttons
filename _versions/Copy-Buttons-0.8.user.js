...
!(function() { // ʕ·͡ᴥ·ʔ hi & ty <3
    ...
    let edo = '',
        notifications = [],
        loopListener = true,
        url_id = window.location.pathname.split('detailed/')[1]; // Projekt-ID (NICHT KD-NR)
    ...
    const loadFromLocalStorage = () => { // Funktion, zum laden der Settings, aus dem lokalen Speicher
        ...
        const storedSettings = localStorage.getItem('settings'); // (versucht) "settings" aus dem lokalen Speicher zu ziehen
    ...
    const createCopyButton = (server, id, url) => {
        if (!document.querySelector('#copyButton')) {
            let path = '';
            path = settings.copy_work_button ? `${settings.copy_drive.trim().toUpperCase()}:\\${server}\\${id}\\work\\` : `${settings.copy_drive.trim().toUpperCase()}:\\${server}\\${id}`;
    ...
    const createSettings = () => {
        ...
        style.textContent = `
            ...
            .cb_container .cb_settings .cb_setting label.cb_switch .box::after {
                ...
                transition: all 0.25s ease-in-out,
                left 0.35s cubic-bezier(0.25, 1, 0.5, 1.55);
            }
            ...
            .cb_container .cb_settings .cb_setting input[type="text"] {
                background: var(--cb_marking_clr) !important;
                border-radius: calc(var(--cb_switch_height) / 3) !important;
                padding: 0px 5px;
                text-align: center;
            }
            ...
            .cb_container .cb_settings .cb_setting .popup_container .popup_content label.cb_input::after {
                ...
                transform: translate(-50%, -50%);
                /* Center horizontally and vertically */
                transition: all 0.25s ease, bottom 0.35s cubic-bezier(0.25, 1, 0.5, 1.15);
            }
            ...
            .cb_container.open .cb_settings {
                transform: scale(1);
                transition: bottom 0.35s cubic-bezier(0.25, 1, 0.5, 1.15),
                            transform 0.35s cubic-bezier(0.25, 1, 0.5, 1.15),
                            background-color 0.25s ease-in-out;
            }
            .cb_container .cb_ls_container {
                position: absolute;
                display: flex;
                flex-direction: column;
                align-items: center;
            ...
            .cb_container .cb_ls_container .cb_ls_close_btn {
                position: absolute;
                padding: 15px;
                top: 0;
                right: 0;
            ...
            .cb_container .cb_ls_container .cb_ls_content {
                display: flex;
                flex-direction: column;
                width: 100%;
                max-height: 0;
                margin-bottom: 10px;
                overflow: auto;
                transition: max-height 0.35s cubic-bezier(0.25, 1, 0.5, 1.15);
                transition-delay: 150ms;
            }
            .cb_container .cb_ls_container .cb_ls_content::-webkit-scrollbar {
                width: 13px;
            }
            .cb_container .cb_ls_container .cb_ls_content::-webkit-scrollbar-track {
                background: var(--cb_background);
                border: 3px solid transparent;
            }
            .cb_container .cb_ls_container .cb_ls_content::-webkit-scrollbar-thumb {
                background: var(--cb_font);
                border: 4px solid transparent;
                border-radius: 100px;
                background-clip: content-box;
            }
            .cb_container .cb_ls_container .cb_ls_content::-webkit-scrollbar-button {
                height: 0;
                width: 0;
            }
            .cb_container .cb_ls_container .cb_ls_content label {
                padding: 10px 0;
                margin-bottom: unset;
                color: var(--cb_font_active);
                text-align: center;
            ...
            .cb_container .cb_ls_container .cb_ls_content label:has(input:checked) {
                background: var(--cb_marking_clr);
                color: var(--cb_font);
            }
            .cb_container .cb_ls_container .cb_ls_content label input {
                display: none;
            }
            .cb_container .cb_ls_container.open {
                transform: scale(1);
                transition: transform 0.35s cubic-bezier(0.25, 1, 0.5, 1.15),
                            background-color 0.25s ease-in-out;
            }
            .cb_container .cb_ls_container.open .cb_ls_content {
        ...
        cbContainer.innerHTML = `
            <div class="cb_settings">
                <div class="cb_ls_container">
                    <span class="cb_ls_close_btn glyphicon glyphicon-remove" title="Übersicht schließen"></span>
                    <span class="settings_title">projekte</span>
                    <div class="cb_ls_content">
                    </div>
                ...
                <div class="cb_setting">
                    ...
                    <label class="cb_switch" for="copy_work_button">
                        <span class="setting_title">"work"-Pfad <span class="glyphicon glyphicon-info-sign cb_info-sign" title="Pfad vom Copy-Button führt bis zum work-Ordner."></span></span>
                        <span class="box"></span>
                    </label>
                </div>
                ...
                <div class="cb_setting">
                    ...
                    <label class="cb_switch" for="copy_icon">
                        <span class="setting_title">"copy"-Icon <span class="glyphicon glyphicon-info-sign cb_info-sign" title="Copy-Icon als Button-Text, anstatt des Pfades."></span></span>
                        <span class="box"></span>
                    </label>
                </div>
        ...
        function loadLocalStorageItems() { // Funktion für den localStorage-Manager
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (/^\d+$/.test(key)) { // Checken, ob "Key" nur aus Nummern besteht
                    ...
                    label.innerHTML = `${key} | <a href="/project/detailed/${key}" title="Link zur Projekt-Seite ${key}" target="_blank">Projektlink</a> <span class="glyphicon glyphicon-info-sign cb_info-sign" title="Domain: ${value_obj.client_domain}&#013;KD-ID: ${value_obj.client_id}&#013;PROJ-ID: ${value_obj.url_id}"></span>`;
        ...
        const cb_ls_deleter_btn = document.querySelector('.cb_ls_deleter_btn');
        const cb_ls_close_btn = document.querySelector('.cb_ls_close_btn');
        const cb_ls_container = document.querySelector('.cb_ls_container');
        cb_btn.addEventListener('click', function() {
            if (cb_ls_container.classList.contains('open') || Array.from(popup_content).some(popup => popup.classList.contains('open'))) {
        ...
        document.addEventListener('click', function(event) {
            ...
            const isClickInsideItems = cb_ls_container.contains(event.target);
